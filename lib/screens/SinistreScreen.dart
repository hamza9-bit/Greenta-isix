...
SinistreView({Key key, this.uid, this.readOnly=false, this.iamAgri=false}) : super(key: key);
                        ...
                        child: ListTile(
                          leading: GestureDetector(
                            ...
                            Container(
                              ...
                              child: Text("+${mysinistre.imagesUrl.length}",style:TextStyle(color: Colors.white)),
                            )
                              ],
                            ),
                          ),
                          title: Text("${mysinistre.agriId}",style: TextStyle(fontSize: 18,fontWeight: FontWeight.bold)),
                          subtitle: Text("Parcelle N°${mysinistre.parcelleRef}",style: TextStyle(fontSize: 18)),
                          trailing: Container(
                            width: 100,
                            child:  Row(
                              mainAxisAlignment: MainAxisAlignment.end,
                              children: [
                             if (!iamAgri)  IconButton(
                                  icon: Icon(Icons.send_outlined),
                                  onPressed: () async{
                                    showParcellesModal(context).then((value){
                                      if (value!=null){
                                        databaseService.makeExpertCansee(mysinistre.sinisteid,value);
                                      }
                                    });
                                  },
                                ),
...
class ExpertSelector extends StatelessWidget {
  ...
  const ExpertSelector({Key key,}) : super(key: key);
